	...
	public void onCallMediaState(OnCallMediaStateParam prm) {
		...
		for (int i = 0; i < cmiv.size(); i++) {
			     ...
			     cmi.getStatus() == pjsua_call_media_status.PJSUA_CALL_MEDIA_REMOTE_HOLD))
			{
				...
				// connect ports
				try {
					PJSipApp.ep.audDevManager().getCaptureDevMedia().startTransmit(am);
					am.startTransmit(PJSipApp.ep.audDevManager().getPlaybackDevMedia());
				} catch (Exception e) {
					continue;
				}
			}
...
class MyAccount extends Account {
	...
	public void onRegState(OnRegStateParam prm) {
		PJSipApp.observer.notifyRegState(prm.getCode(), prm.getReason(), prm.getExpiration());
	}
...
class PJSipApp {
	...
	public void init(MyAppObserver obs, String app_dir, boolean own_worker_thread) {
		...
		/* Start. */
        try {
            ep.codecSetPriority("PCMA/8000/1", (short) 255);

            ep.libStart();
            IntVector int_v = ep.transportEnum();
            for (int i = 0; i < int_v.size(); i++) {
                Log.i("Trans ID " + int_v.get(i), "info: "
                        + ep.transportGetInfo(int_v.get(i)).getInfo());
            }
//            CodecInfoVector codecs = ep.codecEnum();
//            for (int i = 0; i < codecs.size(); i++) {
//                Log.i("ID" + i, codecs.get(i).getCodecId());
//                Log.i("Priority" + i, "prio: " + codecs.get(i).getPriority());
//            }
